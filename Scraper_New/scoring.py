...
def milestone(grant_id):
    ...
    return sum(mile_list)/len(mile_list)
...
def timeliness(grant_id):
    ...
    return sum(rep_frame['Overdue'])/12
...
def spent(grant_id):
    ...
    if grant_budget > 0: 
        spent_score = total_spend/float(grant_budget)
    else: 
        spent_score = 0.0
...
def weight_scores(grant_id):
    ...
    weighted_score = (0.6*goals_score + 0.4*milestones_score)*0.5 + 0.25*spent_score + (0.6*narrative_score + 0.4*timeliness_score)*0.25
...
def record_scores(grant_id):
    scores_tuple = (grant_id[0], goal(grant_id), milestone(grant_id), spent(grant_id), narrative(grant_id), timeliness(grant_id), weight_scores(grant_id), grade(grant_id))
    conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=' + str(conn_filename) + ';')
    cursor = conn.cursor()

    tuples = cursor.execute('''INSERT INTO GrantScores (GrantID, GoalsScore, MilestonesScore, SpendingScore, QualityScore, TimelinessScore, OverallWeighted, Grade)
