...
def get_grants2(grantee):
    conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=' + str(conn_filename) + ';')
    cursor = conn.cursor()
    tuples = cursor.execute('''SELECT ta1.GrantID, ta1.GrantID, ta1.GranteeID, ta1.Program, ta1.Classification, ta1.Budget, ta1.Budget
        FROM Grants ta1
        WHERE ta1.Year = 2022 
        AND ta1.GranteeID = ?
        ''', grantee).fetchall()  
...
def weight_scores(unique_grant):
    ...
    spent_score = spent(unique_grant, 1)
...
def record_scores(unique_grant):
    ...
    scores_tuple = (grant_id_year_two, 0, milestones_score, spent_score, narrative_score, timeliness_score, weighted, 0, 3)
    conn = pyodbc.connect(r'Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=' + str(conn_filename) + ';')
    cursor = conn.cursor()
...
unique_grant = get_grants2(544)[0]

grant_id_year_one = get_grants2(544)[0][0]
grant_id_year_two = get_grants2(544)[0][1]
...
narrative_score = narrative(grant_id_year_one, 1)
timeliness_score = timeliness(grant_id_year_one, 1)
spent_score = spent(get_grants2(544)[0], 1)
